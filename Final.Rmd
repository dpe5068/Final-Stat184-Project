...
Our overarching research question for our project was to determine which variables about a movie have the highest positive correlation with its imbd score. An imdb score is a common measure created as an aggregate score of movie critic reviews, so the higher the imdb score, the better reviews that movie has recieved. To answer our research question, we explored several variables in our movie dataset individual, picked those with the most promise, and then charted those selected variables together. In addition, we created a regression tree with said variables to show what choices a machine learning model would make to predict an imdb score.
...
The Second dataset is a similar dataset about Movies with additional columns like runtime and release year.
```{r}
...
dataset_1_raw %>%
  head(5)
```
Since the second dataset (dataset_2_raw) contains several variables with long string values, only useful variables were selected for a quick view of the data.
```{r}
dataset_2_raw %>%
...
To get a detailed view of which movies recieve higher imdbs, it would be useful to create a chart that incorporates multiple variables and then draw conclusions. Below are lines of code that creates a selected dataset we want to use for this graphic from the main dataset. We select only the relevant variable, and filter out unwanted content_ratings like before
...
We knew beforehand that the release year and revenue variable were probably important for the imdb score. But to make sure, here are two graphics that chart each of those variable against imdb scores
```{r}
final_chart_data %>% 
...
It is difficult to say if the content rating in conjuction with other variables play a significant role in the imdb score, but the conclusion made in the previous graphic seem to stand.
...
After exploring several variables in our movie dataset through summary views, graphics, and a machine learning model, we came to the conclusion that a movie's revenue and the year/decade it was released it have the largest correlation with the movie's imdb score. For revenue, the correlation was position, so the more revenue the movie makes, the higher imdb score it tends to have.  The correlation between release decade and imdb score, however, was negative, whichs means the older the movie, the higher the imdb score.  While we have only found correlation between these variables, it is interesting to think about how factors like revenue and release date could influence and alter how well it is recieved/reviewed.